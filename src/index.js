...
function createPlayerBoards(player, playerDiv) {
  for (let row = 0; row < boardSize; row++) {
    for (let col = 0; col < boardSize; col++) {
      ...
      if (player.gameboard.board.has(key) && player.type !== "computer")
...
function checkWin(shipSunk, key, player, opponent) {
  ...
  updateHitMiss.textContent = shipSunk
    ? `${opponent.type}'s ${shipName} was sunk by ${player.type}`
    : `${opponent.type}'s ${shipName} was hit by ${player.type}`;
